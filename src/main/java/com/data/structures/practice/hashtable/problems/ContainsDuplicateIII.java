...
class SolutionTreeSet {
    ...
    public boolean containsNearbyAlmostDuplicate(int[] nums, int indexDiff, int valueDiff) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            //Largest value less than or equal to current value
            Long floor = set.floor(value);
            if(floor != null && value - floor <= valueDiff)return true;
            //Largest value less than or equal to current value
            Long ceiling = set.ceiling(value);
            if(ceiling != null && ceiling-value  <= valueDiff)return true;

            set.add(value);

            //Maintaining Window
            if (i >= indexDiff) {
                long val = nums[i - indexDiff];
                set.remove(val);
            }
        }
        return false;
